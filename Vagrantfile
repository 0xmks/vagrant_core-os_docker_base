...
required_plugins = %w(vagrant-ignition vagrant-winnfsd)
...
Vagrant.configure("2") do |config|
  ...
  # $num_instances で指定した数だけ繰り返しノード作成
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
      ...
      # provision 時に docker-compose をインストール
      ...
      # vagrant up 時に docker-compose up を実行
